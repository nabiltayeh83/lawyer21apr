...
<tr>
    <td class="v-align-middle wd-5p">
        <div class="checkbox checkMain text-center">
            <input type="checkbox" value="project_flats_fees{{@$one->id}}" id="project_expenses{{@$one->id}}" data-id="{{@$one->total_amount}}"
            @if(in_array($one->id ,$invoice_expenses)) checked  @endif  name="projectExpenses[]" class="chkBox projectExpensesBox">
