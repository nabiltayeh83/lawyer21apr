...
<div class="page-top-fixed">
	<div class="container-fluid">
        <div class="px-3 pb-2 p-md-0 d-flex flex-column flex-md-row align-Authorizationitems-md-center justify-content-between">
            <h2 class="page-header mb-1 my-md-3"> {{__('website.edit')}} {{__('website.task')}}</h2>
    ...
    <div class="bg-white">
        <div class="container-fluid">
            <ol class="breadcrumb breadcrumb-alt">
                ...
                <li class="breadcrumb-item active">{{__('website.edit')}} {{__('website.task')}}</li>
            </ol>
        </div>
    </div>
    <div class=" container-fluid">
        <div class="row no-gutters mt-4">
            <div class="col-lg-12">
			    <form action="{{url(app()->getLocale() . '/tasks/' . $item->id)}}" method="post" id="editTaskForm" autocomplete="off" enctype="multipart/form-data">
                    {{csrf_field()}}
                    {{method_field('PUT')}}
...
<div class="form-group mb-3 row">
	...
	<div class="col-md-9">
        <div class="radio radio-success">
            <input type="radio" value="project" @if($item->task_category == 'project') checked  @endif name="task_category" required id="projectCategory">
            <label for="projectCategory">{{__('website.project')}}</label>
            <input type="radio" value="other" @if($item->task_category == 'other') checked  @endif name="task_category" required id="otherCategory">
            <label for="otherCategory">{{__('website.other')}}</label>
        </div>
    </div>
</div>


<div class="form-group mb-3 row hidden" id="selectProject">
			...
			<select class="full-width" data-init-plugin="select2" id="task_type_id" name="task_type_id">
				<optgroup label="{{__('website.select_task_type')}}">
					...
					@foreach(Auth::user()->office_task_types as $one)
						<option {{($item->task_type_id == $one->task_type_id)? 'selected':''}} value="{{@$one->task_type_id}}">{{@$one->task_type->name}}</option>
					@endforeach
					@endif
				</optgroup>
			</select>
			...
			<select class="full-width" data-init-plugin="select2" id="task_status_id" name="task_status_id">
				<optgroup label="{{__('website.select_task_status')}}">
					...
					@foreach(Auth::user()->office_task_status as $one)
						<option {{($item->task_status_id == $one->task_status_id)? 'selected':''}} value="{{@$one->task_status_id}}">{{@$one->task_status->name}}</option>
					@endforeach
					@endif
				</optgroup>
			</select>
		...
		<div class="form-group form-group-default form-group-default-select2">
			...
			<select class="full-width" data-init-plugin="select2" id="priority" name="priority">
				<optgroup label="{{__('website.select_priority')}}">
					<option {{($item->priority == 'normal')? 'selected':''}} value="normal">{{__('website.normal')}}</option>
					<option {{($item->priority == 'urgent')? 'selected':''}} value="urgent">{{__('website.urgent')}}</option>
				</optgroup>
			</select>
		</div>
	</div>
...
<div class="form-group mb-3 row">
	...
	<div class="col-md-7">
		<div class="form-group form-group-default">
			...
			<input type="text" name="end_date"  value="{{@$item->end_date}}" class="form-control hijri-date-input">
		</div>
	</div>
</div>
...
<div class="form-group mb-3 row">
    ...
    <div class="col-md-7">
        <div class="form-group form-group-default">
            ...
            <input type="time" class="form-control next_time" value="{{@$item->task_time}}" name="task_time">
        </div>
    </div>
</div>
        ...
        <div class="form-group form-group-default form-group-default-select2 required">
            ...
            id="task_employees[]" name="task_employees[]">
                ...
                @foreach(Auth::user()->office_employees as $one)
                    <option @if(in_array($one->id, $task_employees)) selected @endif value="{{@$one->id}}">{{@$one->name}}</option>
                @endforeach
                @endif
                </optgroup>
            </select>
					...
					<div class="form-group form-group-default form-group-default-select2">
						...
						<select class="full-width" data-init-plugin="select2" id="remind_type" name="remind_type">
							<optgroup label="{{__('website.choose_reminder_type')}}">
								<option value=""></option>
								<option {{($item->remind_type == 'email')? 'selected':''}} value="email">{{__('website.email')}}</option>
								<option {{($item->remind_type == 'window')? 'selected':''}} value="window">{{__('website.window')}}</option>
								<option {{($item->remind_type == 'whatsapp')? 'selected':''}} value="whatsapp">{{__('website.whatsapp')}}</option>
							</optgroup>
						</select>
					</div>
				</div>
						...
						<select class="full-width" data-init-plugin="select2" id="remind_time_id" name="remind_time_id">
							<optgroup label="{{__('website.choose_reminder_type')}}">
								...
								@foreach($reminer_time as $one)
									<option {{($item->remind_time_id == $one->id)? 'selected':''}} value="{{@$one->id}}">{{@$one->name}}</option>
								@endforeach
								@endif
							</optgroup>
						</select>
			...
			<div class="form-group form-group-default form-group-default-select2 required">
				...
				<select class="full-width" data-init-plugin="select2" id="responsible_employee" name="responsible_employee">
					<optgroup label="{{__('website.choose_responsible_emp')}}">
					    @if(isset(Auth::user()->office_employees))
					@foreach(Auth::user()->office_employees as $one)
						<option {{($item->responsible_employee == $one->id)? 'selected':''}} value="{{@$one->id}}">{{@$one->name}}</option>
					@endforeach
					@endif
					</optgroup>
				</select>
