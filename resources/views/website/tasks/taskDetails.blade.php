        ...
        <div class="px-3 pb-2 p-md-0 d-flex flex-column flex-md-row align-items-md-center justify-content-between">
            ...
            <div class="page-options-btns">
                <div class="btn-group mb-2 m-md-3 mt-md-0 ml-2">
                    ...
                    <button type="button" class="btn btn-default has-icon" onclick="window.location.href='{{url(getLocal(). '/tasks/' . $item->id . '/edit')}}'">
				...
				<a href="{{ url(getLocal(). '/reports/create') }}">
					<button class="btn btn-complete has-icon mb-2 m-md-0">
                                        ...
                                        <div class="card-body pb-1">
                                            <div class="row dashboard-no-gutters">
                								<div class="col-lg-6 secDataProject">
                                                    ...
                                                    <p>
                                                        <strong>{{__('website.task_type')}}</strong><span>{{@$item->task_type->name}}</span>
                                                    </p>
                                                    <p>
                								...
                								<div class="col-lg-6 secDataProject">
                                                    <p>
                                                        <strong>{{__('website.priority')}}</strong><span>{{__('website.'.$item->priority)}}</span>
                                                    </p>
                                                    <p>
                                                        <strong>{{__('website.task_deliver_date')}}</strong>
                                                        <span>{{Arr::get(getDates(substr($item->end_date, 0, 10)), 'hijri_date')}}                                                        </span>
                                                    </p>

                                                    @if(isset($item->task_time))
                                                    <p>
                                                        <strong>{{__('website.time')}}</strong>
                                                ...
                                                <div class="col-lg-12 secDataProject">
                                                    <p><strong>{{__('website.task_employees')}}</strong></p>
                                                    @foreach ($item->employees as $one)
                                                        <span>{{$loop->iteration}} - {{@$one->user->name}}</span><br>
                                                    @endforeach
                                                </div>
                                                @endif
                            ...
                            <div class="card-body p-0">
                        	    ...
                        	    <div class="table-responsive allHours">
                            	    <table class="table table-hover tableWithSearch allHours" id="tableWithSearch">
        						        <thead>
        							        <tr>
        									    <th class="wd-5p no-sort"><div class="checkbox checkMain text-center">#</div></th>
                                        ...
                                        @foreach($item->hours as $one)
                                            <tr>
        								        ...
        								        <td class="v-align-middle wd-15p"><p>{{@$one->hours_count}}</p></td>
                                                <td class="v-align-middle wd-15p">
                                                    <p>{{Arr::get(getDates(substr($one->start_date, 0, 10)), 'hijri_date')}}</p>
                                                </td>
                                            </tr>
                                        @endforeach
                                        @endif
                            	    ...
                            	    <table class="table table-hover tableWithSearch" id="tableWithSearch">
        							    <thead>
        								    <tr>
                                                ...
                                                <th class="wd-35p">{{__('website.date')}} </th>
                                                <th class="wd-30p">{{__('website.report_status')}} </th>
                                                <th class="wd-30p">{{__('website.hours_count')}} </th>
                                            </tr>
                                        </thead>
                                        ...
                                        <tbody>
        								    <tr>
                                                ...
                                                <td class="v-align-middle wd-35p">
                                                    <a href="">{{substr($one->created_at, 0, 10)}}</a>
                                                </td>
                                                <td class="v-align-middle wd-30p">
                                                    <a href="">{{ __('website.' . $one->status) }}</a>
                                                </td>
                                                <td class="v-align-middle wd-30p"></td>
                                            </tr>
        							    </tbody>
        							    @endforeach
        							    @endif
                                        ...
                                        @foreach($item->attachments as $one)
                                            <tr>
                                                ...
                                                <td class="v-align-middle wd-75p">
                                                    <a href="{{$one->attachment_name}}">{{@$one->attachment_name}}</a>
                                                </td>
