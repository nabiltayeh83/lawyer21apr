...
<div class="page-top-fixed">
	<div class="container-fluid">
        <div class="px-3 pb-2 p-md-0 d-flex flex-column flex-md-row align-items-md-center justify-content-between">
            <h2 class="page-header mb-1 my-md-3"> {{__('website.edit')}} {{__('website.staff')}}</h2>
    ...
    <div class="bg-white">
        <div class="container-fluid">
            <ol class="breadcrumb breadcrumb-alt">
                <li class="breadcrumb-item"><a href="{{url(app()->getLocale() . '/staff')}}">
                    {{__('website.staff')}}</a></li>
                <li class="breadcrumb-item active">{{__('website.edit')}} {{__('website.staff')}}</li>
            </ol>
        </div>
    </div>
    <div class=" container-fluid">
        <div class="row no-gutters mt-4">
            <div class="col-lg-12">
			    <form action="{{url(app()->getLocale() . '/staff/' . $item->id)}}" method="post" id="editTaskForm" autocomplete="off" enctype="multipart/form-data">
                {{csrf_field()}}
                {{method_field('PUT')}}
...
<div class="form-group mb-3 row">
	<label for="fname" class="col-md-3 control-label bold fs-14">{{__('website.address')}}</label>
	<div class="col-md-7">
		<div class="form-group form-group-default required">
...
<div class="form-group mb-3 row">
	...
	<div class="col-md-7">
		<div class="form-group form-group-default required">
			...
			<input type="text" name="hour_price" class="form-control" value="{{@$item->hour_price}}" />
		</div>
	</div>
</div>
...
<div class="form-group mb-3 row">
	<label for="fname" class="col-md-3 control-label bold fs-14">{{__('website.roles')}}</label>
	<div class="col-md-7">
		<div class="form-group form-group-default required">
			...
			<select class="full-width" data-init-plugin="select2" id="role_group_id" name="role_group_id">
				<optgroup label="{{__('website.roles')}}">
					...
					@foreach($roles as $one)
					    <option @if($item->role_group_id == $one->id) selected @endif value="{{@$one->id}}">{{ @$one->name }}</option>
					@endforeach
					@endif
				</optgroup>
			</select>
