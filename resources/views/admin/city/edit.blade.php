        ...
        <div class="col-md-12">
            ...
            <div class="portlet light bordered">
                ...
                <div class="portlet-body form">
                    <form method="post" action="{{url(app()->getLocale().'/admin/cities/'.$item->id)}}" enctype="multipart/form-data" class="form-horizontal" role="form">
                    {{ csrf_field() }}
                    {{ method_field('PATCH')}}
                        <div class="form-body">

                            <fieldset>
                                <legend>{{__('cp.thecountry')}}</legend>
                                    ...
                                    <div class="col-md-6">
                                        <select name="country_id" id="country_id" class="form-control">
                                            @foreach($countries as $one)
                                                <option value="{{$one->id}}" 
                                                    {{$item->country_id == $one->id? "selected" : ""}}>
                                                    {{$one->name}}
                                                </option>
                                            @endforeach   
                                        </select>  
                            ...
                            <div class="form-actions">
                                <div class="row">
                                    <div class="col-md-offset-3 col-md-9">
                                        ...
                                        <a href="{{url(getLocal().'/admin/cities')}}" class="btn default">
                                            {{__('cp.cancel')}}
                                        </a>
                                    </div>
                                </div>
